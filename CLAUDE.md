...
src/
  ...
  lib/                firebase.ts (init), users.ts (user doc + invite code), inviteCode.ts, username.ts (unique handle + 30-day cooldown), friends.ts (relationships), reads.ts (buddy reads), activity.ts (event log), moods.ts (curated end-of-session moods), library.ts (shelved books: tbr/read/favorite + helpers), spines.ts (shared spine palette + SpineTone), books.ts (Google Books client + cover helpers), circle.ts (picks from your buddies' libraries — favorites/TBR — for the Home nook), lines.ts (beloved literary lines, shared by Splash + the nook fallback)
...
**Design:** the dark-academia direction is anchored from a Claude Design mockup — Cormorant Garamond / EB Garamond / IBM Plex Mono, terracotta + gold on parchment/espresso. The **Home** nook (`/home`) leads with a warm time-of-day greeting + a `+` (→ `/search`, `state.from:'/home'`), then your active reads ("Reading now"), then a **genuinely relevant "what next"** that degrades in tiers — never fabricated picks: (1) **From your circle** — books your buddies *love or have lined up* (`lib/circle.ts` reads each friend's friend-readable library; a buddy's *reads* aren't readable, their *library* is), each `CircleCard` tapping to `/book/:id?with=uid` so "Read Together" sends straight to them; else (2) **On your list** — your own to-read shelf as a cover strip; else (3) a quiet **literary line** (`lib/lines.ts`, shared with `Splash`). Bottom nav is 4 tabs (**Home · Library · Activity · You**). **Friends is no longer a tab** — it's reached from the **You** tab ("Your reading circle" → `/friends`); its incoming requests fold into the **Activity** badge and Activity's Needs-you.
