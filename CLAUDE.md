...
**Library (the bookshelf):** `/library` shows your shelved books as an actual wooden bookcase — three shelves, **To Read · Read · Favorites** (Goodreads/Fable lists, shown as a shelf). One doc per book at `users/{uid}/library/{bookId}` with a `shelf` field; **favorite implies read** (shows on both shelves). Add/move/remove from Book detail via `AddToLibrary` (a shelf-picker sheet under the read button). Search is **isolated here** (collapsible magnifier, top-right → `/search`); the older entry points (Shelf CTA, Friends "Read" → `?with=`) stay. Rendering: `Bookshelf` is a **static 2D bookcase** (pure CSS/SVG, no 3D — an earlier react-three-fiber version was tried and pulled for looking flat). A drawn walnut case with recessed shelf backs and lit planks; each book is a bound spine whose **colour is washed from its own cover** (a blurred cover layer under a leather tint — no CORS needed since it's `background-image`, not pixel reads), with raised hubs and a gold-stamped title; covers-less books fall back to the muted spine palette. Tapping a spine raises a `BookSpotlight` (the cover, full glory; tap → `/book/:id`). Modals are viewport-centred — `view-enter` uses `backwards` fill so `<main>` keeps no lingering transform (a transform would make it the containing block for `position:fixed`).
...
- **Done — Library + richer profiles:** personal library as a **static 2D bookcase** (`/library`, To Read · Read · Favorites; `users/{uid}/library`, favorite⇒read), spines colour-washed from each cover, `AddToLibrary` on Book detail (small centred menu), search opens `/search` from the Library. Nav reworked: **Friends off the tab bar, into You; Library tab added** (Shelf · Library · Activity · You). Buddy profiles now show the buddy's three shelves. (A 3D react-three-fiber shelf was tried first and pulled — looked flat; the deps are removed.) Build + lint green.
